...
interface VentaFinalDao {
    ...
    @Query("SELECT COUNT(*) FROM ventas_finales WHERE UPPER(metodo_pago) = UPPER(:metodoPago) AND fecha_cierre BETWEEN :inicio AND :fin") suspend fun contarPorMetodoDia(metodoPago: String, inicio: Long, fin: Long): Int
}
